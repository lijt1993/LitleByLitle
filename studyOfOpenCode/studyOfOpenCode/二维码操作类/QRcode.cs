    ...
    public class QRcode
    {
        ...
        public string Create(string str, int size, string path)
        {
            try
            {
                ...
                //Bitmap bt = qrCodeEncoder.Encode(str, Encoding.UTF8);

                //string filename = "~" + path + Guid.NewGuid() + ".jpg";
                //bt.Save(HttpContext.Current.Request.MapPath(filename));

                Bitmap bt;
                //string enCodeString = encode;
                QRCodeEncoder qrCodeEncoder = new QRCodeEncoder();
                qrCodeEncoder.QRCodeVersion = 5;
                bt = qrCodeEncoder.Encode(str, Encoding.UTF8);
